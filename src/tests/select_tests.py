...
class SelectTests(SqlTestCase):
    ...
    def test_columns(self) -> None:
        select = self._factory.select("id", "username").from_("users")
        self.assertSql("SELECT id, username FROM users", select)
        self.assertParams((), select)

    def test_join(self) -> None:
        select = (
            self._factory.select("u.username", "r.role", "c.country")
            .from_(alias("users", "u"))
    ...
    def test_join_inner(self) -> None:
        select = (
            self._factory.select("u.username", "c.country")
            .from_(alias("users", "u"))
    ...
    def test_where(self) -> None:
        select = self._factory.select().from_("users").where(field("id").eq(1))
    ...
    def test_group_by(self) -> None:
        select = (
            self._factory.select(alias(func("COUNT", "id"), "total"))
            .from_("employees")
            .group_by("department")
        )
        ...
        self.assertSql(
            " ".join(
                ["SELECT COUNT(id) AS total", "FROM employees", "GROUP BY department"]
            ),
            select,
        )
        self.assertParams((), select)
    ...
    def test_having(self) -> None:
        ...
        select = (
            self._factory.select("department", alias(salary_sum, "total"))
            .from_("employees")
            .group_by("department")
    ...
    def test_order_by(self) -> None:
        select = self._factory.select().from_("users").order_by("birthday")
    ...
    def test_order_by_direction(self) -> None:
        select = (
            self._factory.select(
                "u.id", "u.username", alias(func("COUNT", "l.id"), "total")
            )
            .from_(alias("users", "u"))
    ...
    def test_offset_limit(self) -> None:
        select = self._factory.select().from_("users").limit(10).offset(100)
