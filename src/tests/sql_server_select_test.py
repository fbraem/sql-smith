...
class SqlServerSelectTests(SqlTestCase):
    ...
    def test_offset_limit(self) -> None:
        select = (
            self._factory.select().from_("users").order_by("id").offset(0).limit(10)
        )
